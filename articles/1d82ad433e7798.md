...
　2つの整数値のベクトル$\mathbf{a}=(a_i)_i \in \mathbb{N}^n$, $\mathbf{b}=(b_j)_j \in \mathbb{N}^m$と，$i$と$j$の輸送コストを$c_{ij} \in \mathbb{R}$とします．このとき，**最適輸送問題における解空間の頂点は整数です**．今回は，この主張の詳細と，最適輸送問題と整数計画版の最適輸送問題との等価性，及び，証明のスケッチについて述べます．
...
> 定理 1.　ベクトルを$\mathbf{a} \in \mathbb{N}^n$, $\mathbf{b} \in \mathbb{N}^m$, $\mathbf{c} = (c_{ij})_{ij} \in \mathbb{R}^{nm}$とします．このとき，最適輸送問題の解は$\mathbf{p} \in \mathbb{Z}^{nm}$と取れます．
...
> 定理 2.
> ベクトルを$\mathbf{a} \in \mathbb{N}^n$, $\mathbf{b} \in \mathbb{N}^m$, $\mathbf{c} = (c_{ij})_{ij} \in \mathbb{R}^{nm}$とします．このとき，
>
> $$
...
$\mathrm{Id}_n$はサイズが$n$の単位行列であり，$\mathbf{1}_n$はサイズが$n$の$1$が並んでいるベクトルです．
