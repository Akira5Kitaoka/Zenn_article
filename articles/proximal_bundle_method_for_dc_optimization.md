---
title: "近接バンドル法を活用した滑らかさを仮定しないDC最適化"
emoji: "📉"
...
- オラクル$g, h, \nabla g, \nabla h$が高価なとき，全体のコストはその**呼び出し回数**で決まる．DCAは部分問題を反復法で解くので，外側の1反復あたり$g, \nabla g$を何度も呼ぶことになる．
...
この置き換えは2つ目の課題にも効く．部分問題の中で$g$を呼ぶ代わりに過去のカットを使い回すので，**1反復あたりのオラクル呼び出しが$g, \nabla g$それぞれ1回に固定される**．DCAのように内側反復で$g, \nabla g$を呼び続けることがない．
...
この書き方には副産物がある．判定に$h$の**値**が現れないので，**アルゴリズム1は$h$を一度も評価しない**．$h$について必要なのはserious stepでの$\nabla h(x^{k+1})$だけであり，$g$と$\nabla g$は毎反復1回ずつ呼ぶ．$h$の評価が高価な問題——期待値をMonte-Carloで近似する場合など——では，この差が効く．
...
が成り立つ（\[[dO2019][dO2019]\] 2節）．逆向きは一般には成り立たない．臨界点は弱い概念だが，アルゴリズムが到達を保証しやすいという実用上の利点がある．なお$(C)$は劣微分$\partial g, \partial h$**全体**についての条件なので，劣勾配を1つ返すオラクルから直接検証することはできない．次の定理は，検証できる量（安定中心の移動量）から$(C)$を結論する形になっている．
...
- **オラクル呼び出しが減る**：\[[dO2019][dO2019], Table 1\]の問題1（$d = 2$）では，同じ最適値$\bar{f} = 2$に到達するのに要した$\nabla g$の評価回数が，PBM1は$20$回，DCA-CPMは$147$回，DCA-LBMは$177$回，PLMは$269$回であった．内側反復で$g, \nabla g$を呼び続けるかどうかの差である．論文がCPU時間だけでなく$\#\nabla g, \#\nabla h$を主要な指標にしているのも，オラクルが高価な状況を想定しているためである．
...
| | アルゴリズム1 \[[dO2019][dO2019]\]　| PBDC \[[JBKM2017][JBKM2017]\] | DCPCA \[[GGMB2018][GGMB2018]\] |
| --- | --- | --- | --- |
...
代償は近似の粗さである．$\check{g}^k - \bar{h}^{k(\ell)}$は$\check{g}^k - \check{h}^k$より$f$の近似としては悪いはずで，反復回数が増えても不思議はない．ところが数値実験ではそうならなかった．$h$の近似を粗くしても反復回数・計算時間・解の質は悪化しなかった，という結果をもって，論文はこの単純化を正当化している（\[[dO2019][dO2019], Sect. 6.1\]）．ただし全面的な優位ではなく，DCPCAは$\nabla g$の評価回数ではPBM1より少ない（代わりに$\nabla h$が多い）（\[[dO2019][dO2019], Sect. 6.1\]）．
...
滑らかさを仮定しない制約付きDC最適化$\min_{x \in X}(g(x) - h(x))$に対する，\[[dO2019][dO2019]\]の近接バンドル法を紹介した．要点は次のとおりである．

- **アイデア**：DCAが$h$を線形化して凸部分問題を作るのに対し，さらに$g$を切除平面モデル$\check{g}^k$（過去のカットの$\max$）で置き換え，安定中心$x^{k(\ell)}$まわりの近接項を加える．結果，毎反復解くのは**1個の狭義凸計画**$(P)$だけになる．$X$が多面体ならQPである．
- **オラクル効率**：1反復あたりの呼び出しは$g, \nabla g$が1回ずつ，$\nabla h$はserious stepのみ，$h$の値は0回である．DCAが内側反復で$g, \nabla g$を呼び続けるのと対照的で，オラクルが高価なときはこの差が全体のコストを決める．
- **定理・結果**：準位集合が有界なら，安定中心の列の任意の集積点は**臨界点**である（定理 3.）．数値実験でも既知の最良値に届かない例がある．
- **先行手法との違い**：$h$を複数の線形化で近似するバンドル法（PBDC・DCPCA）は無制約問題専用で，1反復に複数のQPを解く．$h$を線形化1本に固定して部分問題を凸に保つ割り切りが，制約付きへの拡張と1反復1問題を同時に実現している．そして数値実験上，この粗い近似による損はほとんど観測されていない．
